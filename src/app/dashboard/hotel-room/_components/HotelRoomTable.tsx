...
import { useHotelRoomsQuery } from "@/hooks/react-query/useRoom";
import { QuerySchemaType } from "@/schema/common/pagination";
import { useRoomDialogStore } from "@/store/Dialog/useRoomStore";

import Pagination from "./Pagination";
import { columns, defaultHotelRoom } from "./columns";

export const HotelRoomTable = () => {
  ...
  const { openDialog } = useRoomDialogStore();
  const { data } = useHotelRoomsQuery(queryParams);

  const tableData = data?.hotelRooms ?? [];
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] flex-col space-y-4 overflow-auto rounded-md border p-4 shadow-sm">
      <div className="flex items-center justify-between">
        <Button className="rounded" size={"sm"} onClick={() => openDialog(defaultHotelRoom)}>
          + 新增房間
        </Button>
        <Input
