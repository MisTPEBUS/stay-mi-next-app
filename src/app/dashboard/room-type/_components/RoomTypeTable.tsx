...
import { useHotelRoomTypesQuery } from "@/hooks/react-query/useRoomType";
...
import { columns, defaultRoomType } from "./columns";
...
export const RoomTypeTable = () => {
  ...
  const { openDialog } = useRoomTypeDialogStore();
  const { data } = useHotelRoomTypesQuery(queryParams);

  const tableData = data?.roomTypes ?? [];
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] flex-col space-y-4 overflow-auto rounded-md border p-4 shadow-sm">
      <div className="flex items-center justify-between">
        <Button className="rounded" size={"sm"} onClick={() => openDialog(defaultRoomType)}>
          + 新增房型
        </Button>
        <Input
