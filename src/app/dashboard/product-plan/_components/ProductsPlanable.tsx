...
import { useProductsPlanQuery } from "@/hooks/react-query/useProductPlan";
...
import { columns, defaultProductPlan } from "./columns";

export const ProductPlanTable = () => {
  ...
  const { openDialog } = useProductPlanDialogStore();
  const { data } = useProductsPlanQuery(queryParams);

  const tableData = data?.productPlans ?? [];
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] flex-col space-y-4 overflow-auto rounded-md border p-4 shadow-sm">
      <div className="flex items-center justify-between">
        <Button className="rounded" size={"sm"} onClick={() => openDialog(defaultProductPlan)}>
          + 新增伴手禮計畫
        </Button>
        <Input
